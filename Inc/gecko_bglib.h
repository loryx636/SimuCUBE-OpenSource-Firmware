...
// this function needs to read up to len1 bytes to data1 and return how many bytes were read into the buffer. 0 is ok but does not mean EOF.
extern int32_t (*bglib_input)(uint32_t len1, uint8_t* data1);

extern int32_t(*bglib_peek)(void);

/**
 * This value should only be used to correlate debug logging.
 * @return the current state (opaque value) of the framer state machine
 */
uint8_t bglib_framing_state();
